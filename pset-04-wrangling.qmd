...
#| warning: false
library(httr2)
census_endpoint <- "https://api.census.gov/data/2021/pep/population"
request <- request(census_endpoint) |>
  req_url_query(
...
# convert state name from list to character
population$state_name <- as.character(population$state_name)
```
...
population |>
  mutate(state_year = paste(year, state_name)) |>
  ggplot(aes(x = population,
             y = reorder(state_year, population),
             fill = factor(year))) +
  geom_col() +
  facet_wrap(~ year, scales = "free_x") +
  scale_y_discrete(labels = function(lbl) sub("^\\d{4} ", "", lbl)) +
  labs(x = "Population", y = "State", fill = "Year",
...
regions <- map_df(raw_list, \(r) tibble(
  ...
  mutate(region_name = if_else(
    str_detect(region_name, regex("Affiliated\\s*Pacific", ignore_case = TRUE)),
    "USAPI", region_name)) |>
  # keep only 51 rows
...
cases |>
  ...
  inner_join(
    population |> select(state, year, population, region_name) |> distinct(),
    by = c("state","year")
  ) |>
  filter(!is.na(region_name)) |>
  mutate(
    date = as.Date(date),                   # temporary for plotting
